...
namespace MicroSisPlani
{
    public partial class Frm_Reg_Justificacion : Form
    {
        ...
        private bool ValidarDatos()
        {
            ...
            if (txt_idjusti.Text.Length < 8) { fil.Show(); adv.Lbl_Msm1.Text = "El Id Justificación no fue generado"; adv.ShowDialog(); fil.Hide(); txt_idjusti.Focus(); return false; }

            if (cbo_motivJusti.SelectedIndex == -1) { fil.Show(); adv.Lbl_Msm1.Text = "Selecciona el motivo"; adv.ShowDialog(); fil.Hide(); cbo_motivJusti.Focus(); return false; }
            if (txt_DetalleJusti.Text.Length < 5) { fil.Show(); adv.Lbl_Msm1.Text = "Ingresa breve motivo de la justificación"; adv.ShowDialog(); fil.Hide(); txt_DetalleJusti.Focus(); return false; }
            return true;
        }
        ...
        private void Registrar_Justificacion()
        {
            ...
            try
            {
                ...
                jus.Estado = Cls_Libreria.Apellidos;
                obj.RN_Registrar_Justificacin(jus);
                if (BD_Justificacion.saved == true)
                {
                    RN_Utilitario.RN_Actualiza_Tipo_Doc(4);
                    fil.Show();
                    ok.Lbl_msm1.Text = "Justificación registrada";
                    ok.ShowDialog();
                    fil.Hide();
                    LimpiarPiso();
